...
# Phishing ability: Attack opponent card or HP, then steal 1 card from deck
const DAMAGE_DEALT = 2
const CARDS_TO_STEAL = 1
...
func trigger_ability(battle_manager_reference, card_with_ability, input_manager_reference):
	...
	# Steal cards from opponent deck
	var opponent_deck_reference = battle_manager_reference.get_parent().get_node_or_null("OpponentDeck")
	if opponent_deck_reference and opponent_deck_reference.opponent_deck.size() > 0:
		for i in range(CARDS_TO_STEAL):
			if opponent_deck_reference.opponent_deck.size() > 0:
				var stolen_card = opponent_deck_reference.opponent_deck.pick_random()
				opponent_deck_reference.opponent_deck.erase(stolen_card)
				# Add stolen card to player deck
				var player_deck_reference = battle_manager_reference.get_parent().get_node_or_null("Deck")
				if player_deck_reference:
					player_deck_reference.player_deck.append(stolen_card)
	
	await battle_manager_reference.wait(1.0)
	
	# Destroy the Phishing card after ability resolves
